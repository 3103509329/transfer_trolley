...
//            }
    ...
    public MessageResp<AuthUserResp> getUserInfoByToken(HttpServletRequest request){
        String token = request.getHeader("token");
        MessageResp resp = new MessageResp();
        AuthUserResp user = sessionHandler.getUser(token);
        resp.setData(user);
        return resp;
    }


    @PostMapping("/loginWebPost")
    public @ResponseBody
    MessageResp<AuthUserResp> loginWebPost(HttpServletRequest request,
        ...
        if (temp == 1) {
            ...
            resp.setData(user);
            resp.setResMap(map);
            sessionHandler.setUser(jwtToken, user);
        } else if (temp < 0) {//账号或密码不正确
            resp.setStatusCode("-50");
            resp.setResult("false");
            resp.setResultDesc("用户名或密码不正确！");
