package com.zhcx.authorization.controller.netcar.mapvideo;

import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.JSONObject;
import com.google.gson.Gson;
import com.zhcx.authorization.utils.*;
import com.zhcx.netcarbasic.constant.ThirdPartyURL;
import com.zhcx.netcarbasic.pojo.mapAndVideo.*;

import com.zhcx.platformtonet.facade.YunZhengCompanyService;
import com.zhcx.platformtonet.facade.YunZhengVehicleService;
import com.zhcx.platformtonet.pojo.base.Thirdparty_VehicleAndDriver;
...
public class MapAndVideoServiceController {
    ...
    public MessageResp getCurrentLocation(HttpServletRequest request,
        ...
        try {
            ...
            JSONObject dataResult = JSONObject.parseObject(thirdparty_result);
            if (null != dataResult.get("vehicles")) {
                ...
                for (YunZhengVehicle vehicle : vehicleList) {
                    ...
                    JSONObject locationResult = JSONObject.parseObject(requestData);
    ...
    public MessageResp getTrack(HttpServletRequest request,
        ...
        try {
            if (StringUtils.isNotEmpty(vehiIdno)) {
                ...
                result = new Gson().fromJson(httpResult , MapRsult.class);
            }
            messageResp.setData(result.getTracks());
            messageResp.setResult(Boolean.TRUE.toString());
            messageResp.setResultDesc("查询成功");
        } catch (Exception e) {
            log.error(e.getMessage());
    ...
    private Object getEquipmentNumber(String vehiIdno, String jsession) throws Exception {
        ...
        String httpResult = HttpClientHelper.sendGet(url, map);
        httpResult = judgeResult(httpResult, url, map);
        Map<String, Object> result = JSONObject.parseObject(httpResult);
