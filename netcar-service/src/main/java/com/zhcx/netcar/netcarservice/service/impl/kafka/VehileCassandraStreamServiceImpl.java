 ...
 **/
@Component
public class VehileCassandraStreamServiceImpl implements KafkaStreamService {
    private static final Logger log = LoggerFactory.getLogger(VehileCassandraStreamServiceImpl.class);
    private KafkaStreams streams = null;

    @Value("${netcar.cassandra.vehile}")
    private String STREAM_TOPIC_IN;

    @Value("${netcar.cassandra.vehile.out}")
    ...
    private KafkaStreams startKStreams(final String bootstrapServers, final String stateDir) {
        ...
        KStream<String, String> max = source.mapValues(value -> {
            ...
            try{
                NetcarPositionVehicle vehicle = JSON.parseObject(value, NetcarPositionVehicle.class);

                SimpleDateFormat sdf=new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
                String date = sdf.format(new Date(Long.parseLong(vehicle.getPositionTime())*1000));
                Date data = DateUtil.getYMDHMSFormat(date);
                map.put("entity_type", CassandraEntityTypeEnum.VEHICLE.getCode());
                map.put("entity_id", vehicle.getVehicleNo());
                map.put("key", "FULL");
                map.put("str_v", JSON.toJSONString(vehicle));
