 ...
 **/
@Component
public class LogOutStreamServiceImpl implements KafkaStreamService {
    private static final Logger log = LoggerFactory.getLogger(LogOutStreamServiceImpl.class);
    private KafkaStreams streams = null;

    @Value("${netcar.stream.topic.logout.in}")
    ...
    private KafkaStreams startKStreams(final String bootstrapServers, final String stateDir) {
        ...
        KStream<String, String> max = source.mapValues(value -> {
            ...
            try{
                NetcarOperateLogout loginList = JSON.parseObject(value, NetcarOperateLogout.class);
                Date payTime = DateTimeUtil.StringToDate(loginList.getLogoutTime(),"yyyyMMddHHmmss");
