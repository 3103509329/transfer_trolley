...
package com.zhcx.netcar.netcarservice.controller.gps;
...
import com.zhcx.netcar.netcarservice.utils.PageBean;
...
public class PositionController {
    ...
    @PostMapping("/order/queryHistoryPosition")
    @ApiOperation(value = "网约车历史轨迹（ 指定订单 ）", notes = "网约车历史轨迹")
    public MessageResp queryOrderHistoryPosition(@RequestBody VehiclePositionParam vehiclePositionParam) {
        MessageResp messageResp = new MessageResp();
        List<NetcarPositionVehicle> list = new ArrayList<>();
        try {
            if (StringUtils.isBlank(vehiclePositionParam.getVehicleNo())) {
                return CommonUtils.returnErrorInfo("车辆号牌不能为空");
            }
            //获取指定时间区间的轨迹数据，时间区间来源于订单的开始时间和结束时间
            list = positionService.queryOrderHistoryPosition(vehiclePositionParam);
            messageResp.setResultDesc("查询成功");
            messageResp.setData(list);
        } catch (Exception e) {
            log.error(e.getMessage());
