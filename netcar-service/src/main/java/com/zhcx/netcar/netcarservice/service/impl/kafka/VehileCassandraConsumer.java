 ...
 */
@Component
public class VehileCassandraConsumer {
    private static final Logger log = LoggerFactory.getLogger(VehileCassandraConsumer.class);
    @Autowired
    private PositionRepository positionRepository;

    @KafkaListener(topics = "${netcar.cassandra.vehile.out}")
    public void alarmConsume(String msgContent) {
        ...
        try {
            ...
            log.info("车辆轨迹数据入库完成" + ts_kv_cf);
            log.info("车辆轨迹数据入库完成: " + "时间" + ts_kv_cf.getTs() + "   分区：" + partition);
            ...
            e.printStackTrace();
            log.error("车辆定位入库canssandra异常：" + String.valueOf(e.getMessage()));
        }
    }
}
